...
class Variational_Actor(nn.Module):
    """
    Actor network class for HAPPO. Outputs actions given observations.
    :param args: (argparse.Namespace) arguments containing relevant model information.
    :param obs_space: (gym.Space) observation space.
    ...
    def __init__(self, args, obs_space, action_space, device=torch.device("cpu")):
        ...
        self.pre_act = ACTLayer(action_space, 2 * self.hidden_size, self._use_orthogonal, self._gain, args)

        self.other_act_encoder = MLPLayer(other_action_space, self.hidden_size, 1,  self._use_orthogonal, args.use_ReLU)
    ...
    def evaluate_actions(self, obs, rnn_states, action, masks, available_actions=None, active_masks=None, other_actions=None):
        """
        Compute log probability and entropy of given actions.
        :param obs: (torch.Tensor) observation inputs into network.
        :param action: (torch.Tensor) actions whose entropy and log probability to evaluate.
    ...
    def evaluate_actions_info(self, obs, rnn_states, action, masks, old_action_log_probs, available_actions=None, active_masks=None):
        """
        Compute log probability and entropy of given actions.
        :param obs: (torch.Tensor) observation inputs into network.
        :param action: (torch.Tensor) actions whose entropy and log probability to evaluate.
        ...
        if self.args.algorithm_name=="hatrpo":
            ...
            old_action_log_probs_factor = torch.exp(action_log_probs-old_action_log_probs)
            return old_action_log_probs_factor, action_log_probs, dist_entropy
